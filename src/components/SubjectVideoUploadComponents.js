...
const SubjectVideoUploadComponents = React.createClass({
    getInitialState() {
        submitFileOptions = [];
        return {
            visible: false,
    ...
    checkFileInfo(files) {
        if (files.length <= 0) {
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                var isMuliti = this.checkSubmitFileCount(length);
                if (isMuliti == true) {
                    ...
                    message.warning("本系统只支持后缀名为mp3文件的上传操作,请重新上传,谢谢!", 10);
            ...
            this.setState({submitFileCheckedList: [], submitFileOptions: submitFileOptions});
            //回调，将已上传的文件列表传给题目上传组件
            this.props.callBackParent(submitFileOptions);
        }
    },

    //判断已上传文件个数，目前只允许单文件上传
    checkSubmitFileCount(length) {
        var isOk = false;
        // alert(this.state.submitFileOptions.length)
        // alert(length);
...
export default SubjectVideoUploadComponents;
