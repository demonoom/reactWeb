...
var RichEditorComponentsForAnswer = React.createClass({
  componentDidMount(){
    editor = UE.getEditor("answerContainer", {
      //工具栏
      toolbars: [[
        'undo', 'redo', '|',
...
/*  componentWillMount(){
    console.log("答案componentWillMount");
    if(editor==null || typeof(editor)=="undefined" ){
      editor = new UE.Editor();
      editor.render("answerContainer");
      pasterMgr.Init(editor);
    }
  },*/

  componentWillUnmount(){
    console.log("答案unmount");
    //React.unmountComponentAtNode(document.getElementById("singleContainer"));
    UE.getEditor("answerContainer").destroy();
  },
