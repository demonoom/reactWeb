...
// import UploadImgComponents from '../dingMessage/UploadImgComponents';
...
class EditDingModal extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.setState({isShow});
        if (isEmpty(nextProps.msgContent) == false) {
            var msgContent = nextProps.msgContent;
            this.setState({sendMes: msgContent})
            var dingSelectedRowKeys = nextProps.dingSelectedRowKeys;
        }
        if (isEmpty(nextProps.dingSelectedRowKeys) == false) {
            var dingSelectedRowKeys = nextProps.dingSelectedRowKeys;
            this.setState({selectedRowKeys: dingSelectedRowKeys});
        }
        if (isEmpty(nextProps.dingSelectedNames) == false) {
            var dingSelectedNames = nextProps.dingSelectedNames;
            this.setState({tags: dingSelectedNames});
        }
        if (isEmpty(nextProps.dingUuid) == false) {
            var dingUuid = nextProps.dingUuid;
            this.setState({dingUuid});
        }
    ...
    sendDing() {
        ...
        var _this = this;
        var param = {
            "method": 'createMessageBiu',
            "userId": this.state.loginUser.colUid,
            "messageUUID": this.state.dingUuid,
            "biuType": mesType,
