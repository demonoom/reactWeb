...
;(function ($) {
    ...
    littlePanle.prototype.closepanle = function (id) {
        ...
        let tmp = [];
        TP.mgr.map(function (item, index) {
            if (item.id == id) {
                tmp.push(item)
            }
        });
        TP.hideArr = TP.hideArr.concat(tmp);
        if (!(TP.mgr.length - TP.hideArr.length)) {
            TP.delAll();
        } else {
        ...
        this.htm = `<div id="${id}" class="dialog little-layout-aside-r-show teachingAdmin">
                ...
                <h3 class="title" id="${this.ifrid}_title">${ obj.title }</h3>
                    <div class="little-tilte">
                        ...
                        <a class="share" id="${this.ifrid}_share"><i class="anticon anticon-share-alt "></i></a>
                    </div>
                </div>
                <div class="content">
...
// 后进的学生，显示的ppt
    ...
    window.__noomPanel__ = function (objParam) {
        //调用原来littlePanel调用GETLP的事
        TP.Start(objParam);
    }
...
<div role="document" class="ant-modal " style="width: 616px;"><div class="ant-modal-content">
