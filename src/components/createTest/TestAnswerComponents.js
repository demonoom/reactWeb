...
const TestAnswerComponents = React.createClass({
    ...
    buildChoiceCard(questionId, number, score, cardArray, resultsArray) {
        ...
        var everyRow = <Card key={questionId} className="upexam_topic">
            ...
            <Row>
                <Col span={24}>
                    <CheckboxGroup options={selectAnswerOptions} defaultValue={charArray}
                                   onChange={_this.subjectAnswerOnChange}/>
                </Col>
            </Row>
        </Card>;
    ...
    buildFillBlankCard(questionId, number, score, cardArray, resultsArray) {
        ...
        var everyRow = <Card key={questionId} className="upexam_topic">
            ...
            <Row>
                ...
                <Col span={20}>
                    <Input id={questionId + "#Input"} placeholder="输入答案" defaultValue={textAnswer}
                           onChange={_this.blankAnswerOnChange}/>
                </Col>
            </Row>
    ...
    buildExmSubmitResultJsonArray(exmSubmitResultJsonForAdd, answerType) {
        ...
        if (isExistSameQuestion == false && !!exmSubmitResultJsonForAdd.questionId) {
