...
const UpdateExamPagerComponents = React.createClass({
    ...
    buildCardChildArray(){
        cardChildTagArray = cardChildArray.map((e, i)=> {
            ...
            return <Card key={e.answerTitle+"#"+e.answerSubjectType} title={e.answerTitle+"("+type+")"} extra={<button title={e.answerTitle} value={e.answerTitle+"#"+e.answerSubjectType} icon="delete" onClick={createExamPager.showDelAnswerCardConfirmModal}  className="btn_gray_exam examination_btn_gray"><i className="iconfont btn_gray_exam_del">&#xe62f;</i></button>} className="upexam_topic">
                {
    ...
    showBindKnowledgeModal(e) {
        ...
        createExamPager.setState({bindKnowledgeBtnInfo:subjectInfo,bindKnowledgeModalVisible: true,defaultSelected:[]});
    },
    /**
     * 获取当前图片已选定或输入的信息
     * @param subjectInfo
                ...
                <span className="date_tr text_30 upexam_float">
                    ...
                    <Row className="date_tr ant-form-item">
                        ...
                        <Col span={12}>
                            ...
                            <button type="ghost" htmlType="reset" className="login-form-button examination_btn_white" onClick={createExamPager.showDelAllAnswerCardConfirmModal}>
                                清除全部
                            </button>
                        </Col>
                    </Row>
